...
def index(request):
    ...
    context = {'rows': rows, 'fil_des': fil_des,
               'par_app': par_app, 'user_perm': user_perm}
...
# (sys_menu.add_menu,sys_menu.change_menu) sem permissão, retorna para index
@permission_required(('sys_'+_app_name+'.add_'+_app_name, 'sys_'+_app_name+'.change_'+_app_name), login_url='index')
...
@login_required(login_url='login')
# (sys_menu.delete_menu) sem permissão, retorna para index
@permission_required('sys_'+_app_name+'.delete_'+_app_name, login_url='index')
def delete(request, pk=0):
...
def index_grupo(request):
    par_app, user_perm, _render = init(request, 'group', 'auth')
    ...
    if len(fil_des) > 0:
        rows = rows.filter(name__contains=fil_des)
    else:
        rows = rows.all()
...
@login_required(login_url='login')  # sem login, retorna para login
# sem permissão, retorna para index
@permission_required(('auth.add_group', 'auth.change_group'), login_url='index')
